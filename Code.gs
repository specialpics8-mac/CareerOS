...
function handleRequest_(e) {
  ...
  try {
    ...
    else if (action === "uploadResumeFile") result = uploadResumeFile_(e.parameter);
    else if (action === "deleteResumeFile") result = deleteResumeFile_(e.parameter);
...
// Matches the API contract used by CareerOS index.html.
// Files are stored in a private Google Drive folder.
// ═══════════════════════════════════════════════════════════

const RESUME_LIBRARY_FOLDER_NAME = "CareerOS Resume Library";
const MAX_RESUME_FILE_BYTES = 5 * 1024 * 1024; // 5 MB

function getResumeLibraryFolder_() {
  const folders = DriveApp.getFoldersByName(RESUME_LIBRARY_FOLDER_NAME);
  if (folders.hasNext()) return folders.next();
  return DriveApp.createFolder(RESUME_LIBRARY_FOLDER_NAME);
}
...
function uploadResumeFile_(p) {
  ...
  const type = String(p.type || "Resume").trim();
  const targetRole = String(p.targetRole || "").trim();
  const targetCompany = String(p.targetCompany || "").trim();
  const notes = String(p.notes || "").trim();

  if (!name) throw new Error("Please select a file.");
  if (!base64) throw new Error("File data was not received.");
  ...
  const folder = getResumeLibraryFolder_();
  const blob = Utilities.newBlob(bytes, mimeType, name);
  const file = folder.createFile(blob);
  file.setDescription("CareerOS Resume & Document Library");
  ...
  getSheet_(SHEETS.RESUME_LIBRARY).appendRow([
    record.ID,
    record.Name,
    record.Type,
    record.FileId,
...
function deleteResumeFile_(p) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][0]) === id) {
      const fileId = String(data[i][3] || "");

      if (fileId) {
        try {
          DriveApp.getFileById(fileId).setTrashed(true);
        } catch (err) {
          // If the file is already deleted/missing, still clean up the record.
        }
      }
